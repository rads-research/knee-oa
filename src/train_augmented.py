"""
Train augmentation-based (E1) models for knee OA classification.

Identical to baseline training except each training image has a 40%
probability of receiving a randomly selected artifact pattern with
severity uniformly sampled between 50% and 150%.

CRITICAL: Augmentation is applied ONLY to training images.
...
from artifacts import ArtifactPatterns

# Configuration (matches manuscript Section 2.5)
...
PATIENCE = 15
AUG_PROB = 0.4
AUG_SEVERITY_RANGE = (0.5, 1.5)


def train_augmented_model(seed, train_root, val_root, save_path, device):
    """
    Train one augmentation-based model.

    Same architecture and hyperparameters as baseline. The only
    modification is artifact augmentation during training.
    """
    print(f"\n  Seed: {seed}")
    ...
    # Augmentation function — applied ONLY to training images
    def augment_fn(img):
        return ArtifactPatterns.apply_random_augmentation(
            img, prob=AUG_PROB, severity_range=AUG_SEVERITY_RANGE
        )

    # Training dataset WITH augmentation
    train_ds = KneeOADataset(train_root, augment_fn=augment_fn)
    ...
    print(
        f"  Training with artifact augmentation "
        f"(prob={AUG_PROB}, severity={AUG_SEVERITY_RANGE})"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Train augmented (E1) models")
    ...
    model_dir = os.path.join(args.output_dir, "models", "augmented")
    ...
    print("=" * 60)
    print("AUGMENTATION-BASED MODEL TRAINING (E1)")
    print(f"  Augmentation probability: {AUG_PROB}")
    print(f"  Severity range: {AUG_SEVERITY_RANGE}")
    print(f"  Applied to: training images ONLY")
    print(f"  Validation: NO augmentation")
    print("=" * 60)

    for seed in SEEDS:
        model_name = f"E1_s{seed}"
